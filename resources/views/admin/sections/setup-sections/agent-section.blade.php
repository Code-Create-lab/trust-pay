...
@section('content')
                        ...
                        <div class="product-tab">
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                <div class="tab-pane @if (get_default_language_code() == language_const()::NOT_REMOVABLE) fade show active @endif" id="english" role="tabpanel" aria-labelledby="english-tab">
                                    ...
                                    {{-- //start bottom section informatiom --}}
                                    <div class="form-group">
                                        @include('admin.components.form.input',[
                                            'label'     => __("Bootm Heading")."*",
                                            'name'      => $default_lang_code . "_bottom_heading",
                                            'value'     => old($default_lang_code . "_bottom_heading",$data->value->language->$default_lang_code->bottom_heading ?? "")
                                        ])
                                    </div>
                                    <div class="form-group">
                                        @include('admin.components.form.input',[
                                            'label'     => __( "Bottom Sub Heading")."*",
                                            'name'      => $default_lang_code . "_bottom_sub_heading",
                                            'value'     => old($default_lang_code . "_bottom_sub_heading",$data->value->language->$default_lang_code->bottom_sub_heading ?? "")
                                        ])
                                    </div>
                                    <div class="form-group">
                                        @include('admin.components.form.textarea',[
                                            'label'     => __( "Bottom Details")."*",
                                            'name'      => $default_lang_code . "_bottom_details",
                                            'value'     => old($default_lang_code . "_bottom_details",$data->value->language->$default_lang_code->bottom_details ?? "")
                                        ])
                                    </div>
                                </div>
                                ...
                                @foreach ($languages as $item)
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="{{ $item->name }}" role="tabpanel" aria-labelledby="english-tab">
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input',[
                                                'label'     => __( "Bottom Heading")."*",
                                                'name'      =>$item->code . "_bottom_heading",
                                                'value'     => old($item->code . "_bottom_heading",$data->value->language->$lang_code->bottom_heading ?? "")
                                            ])
                                        </div>
                                        <div class="form-group">
                                            @include('admin.components.form.input',[
                                                'label'     => __( "Bottom Sub Heading")."*",
                                                'name'      => $item->code . "_bottom_sub_heading",
                                                'value'     => old($item->code . "_bottom_sub_heading",$data->value->language->$lang_code->bottom_sub_heading ?? "")
                                            ])
                                        </div>
                                        <div class="form-group">
                                            @include('admin.components.form.textarea',[
                                                'label'     => __( "Bottom Details")."*",
                                                'name'      => $item->code . "_bottom_details",
                                                'value'     => old($item->code . "_bottom_details",$data->value->language->$lang_code->bottom_details ?? "")
                                            ])
                                        </div>
                                    </div>
                                @endforeach
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#merchant-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i> {{ __("Add Item") }}</a>
                </div>
            </div>
    ...
    @include('admin.components.modals.site-section.add-merchant-item')

    {{-- Solution Item Edit Modal --}}
    <div id="merchant-edit" class="mfp-hide large">
...
@push('script')
 <script src="{{ asset('backend/js/fontawesome-iconpicker.js') }}"></script>
    ...
    <script>
        openModalWhenError("merchant-add","#merchant-add");
        openModalWhenError("merchant-edit","#merchant-edit");
        ...
        $(".edit-modal-button").click(function(){
            ...
            openModalBySelector("#merchant-edit");
