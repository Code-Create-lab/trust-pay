...
@section('content')
                    ...
                    <tbody>
                        ...
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td>{{ $item->language->$system_default_lang->question ?? "" }}</i></td>
    ...
    @include('admin.components.modals.site-section.add-developer-faq-item')

    {{-- Developer FAQ Item Edit Modal --}}
    ...
    <script>
        ...
        $(".edit-modal-button").click(function(){
            ...
            $.each(languages,function(index,item) {
                editModal.find("input[name="+item.code+"_question_edit]").val(oldData.language[item.code]?.question);
                editModal.find("textarea[name="+item.code+"_answer_edit]").val(oldData.language[item.code]?.answer);
