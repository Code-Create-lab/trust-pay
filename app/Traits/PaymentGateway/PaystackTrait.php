...
trait PaystackTrait
{
    ...
    public function setupPaystackInitAddMoney($output,$credentials){
        ...
        if(userGuard()['guard']  === 'web'){
            ...
            $return_url = route('agent.add.money.payment.callback.response');
        }
    ...
    public function setupPaystackInitPayLink($output,$credentials){
        ...
        $return_url = route('payment-link.gateway.payment.callback.response');
        ...
        $response   = json_decode($result);
        if($response->status == true) {
            $this->paystackJunkInsertPayLink($response,$response->data->reference,$credentials);
            return redirect($response->data->authorization_url)->with('output',$output);
        }else{
            throw new Exception($response->message??" "."Something Is Wrong, Please Contact With Owner");
        }
    ...
    public function paystackJunkInsertPayLink($response,$temp_record_token,$credentials) {
        $output = $this->output;
        $wallet_table = $output['wallet']->getTable();
        $wallet_id = $output['wallet']->id;
    ...
    public function paystackInitApi($output = null) {
        ...
        if(authGuardApi()['guard'] == 'agent_api'){
            ...
            $user_email = $user->email;
            $return_url = route('api.agent.add.money.payment.callback.response', "r-source=" . PaymentGatewayConst::APP);
        ...
        if($response->status == true) {
            ...
            $error = ['error'=>$response->message??" ".[__("Something went wrong! Please try again.")]];
